...
#      (mean_logro, pred_final_logro, slope_logro y su propio
#      n_evals; colegio_efecto_historico es la misma para todos
#      los estudiantes de un mismo colegio, porque describe al
#      colegio, no al estudiante).
#
# Supuesto clave de la predicción individual: como el modelo es
# lineal en variables que significan lo mismo a ambos niveles
# (promedio de logro, trayectoria proyectada, pendiente), se asume
# que la relación ensayo -> SIMCE es aproximadamente la misma
# dentro de un colegio que entre colegios. Es un punto de partida
# razonable mientras no exista un SIMCE individual real para
# calibrar directamente un modelo a nivel de estudiante (el SIMCE
# oficial sólo se reporta agregado por colegio). Ver README.md,
# sección "Limitaciones y siguientes pasos".
#
...
write_csv(
  pred_colegio %>%
    select(agno, grado, area, rbd_revisado, n_estudiantes, mean_logro,
           pred_final_logro, colegio_efecto_historico, pred_simce),
...
write_csv(
  pred_individual %>%
    select(agno, grado, area, rbd_revisado, id_usuario_curso, mean_logro,
           pred_final_logro, colegio_efecto_historico, pred_simce),
  dir_salidas %>% file.path("predicciones_individual.csv")
)
